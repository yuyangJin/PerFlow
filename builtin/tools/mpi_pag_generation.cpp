...
int main(int argc, char** argv) {
  ...
  for (int pid = 0; pid < num_procs; pid++) {
    std::string perf_data_file_name = std::string(data_dir) + std::string("/dynamic_data/SAMPLE+") + std::to_string(pid) + std::string(".TXT");
  ...
  std::string output_static_pag_name = std::string(data_dir) + std::string("/static_pag.gml");
  pag->DumpGraphGML(output_static_pag_name.c_str());

  /** Data embedding */
  std::map<baguatool::type::procs_t, baguatool::collector::SharedObjAnalysis *> all_shared_obj_analysis;
  for (int pid = 0; pid < num_procs; pid++) {
    std::string somap_file_name_str = std::string(data_dir) + std::string("/dynamic_data/SOMAP+") + std::to_string(pid) + std::string(".TXT");
    baguatool::collector::SharedObjAnalysis *shared_obj_analysis = new baguatool::collector::SharedObjAnalysis();
  ...
  graph_perf->GenerateDynAddrDebugInfo(perf_data, all_shared_obj_analysis, bin_name_str);
  ...
  std::string output_pag_perf_data_name = std::string(data_dir) + std::string("/output.json");
  ...
  graph_perf->GenerateDynAddrDebugInfo(comm_data, all_shared_obj_analysis, bin_name_str);

  graph_perf->AddCommEdgesToMPAG(comm_data);

  auto mpag = graph_perf->GetMultiProgramAbstractionGraph();

  std::string output_pag_to_mpag_map_name = std::string(data_dir) + std::string("/pag_to_mpag.json");
  ...
  std::string output_mpag_perf_data_name = std::string(data_dir) + std::string("/mpag_perf_data.json");
  ...
  std::string output_mpag_name = std::string(data_dir) + std::string("/mpi_mpag.gml");
