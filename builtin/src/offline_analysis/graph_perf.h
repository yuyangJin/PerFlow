...
private:
  std::map<type::addr_t, core::ControlFlowGraph *>
      func_entry_addr_to_cfg;  /**<control-flow graphs for each function*/
  core::ProgramCallGraph *pcg; /**<program call graph*/
  std::map<type::addr_t, core::ProgramAbstractionGraph *>
      func_entry_addr_to_pag; /**<program abstraction graph extracted from
                                 control-flow graph (CFG) for each function */
  ...
  core::ProgramAbstractionGraph
      *root_pag; /**<an overall program abstraction graph for a program */
  core::MultiProgramAbstractionGraph
      *root_mpag; /**<an overall multi-* program abstraction graph for a
                     parallel program*/
...
public:
  ...
  /** Geneate control-flow graph of each function through hybrid static-dynamic
   * analysis. Dynamic module is disable.
   * @param dir_name - dictory name of all functions' CFG
   */
  ...
  void GenerateDynAddrDebugInfo(core::PerfData *perf_data,
  ...
  /**
   * @brief If debug infos of dynamic executables or libraries have been
   * generated.
   *
   * @return true
   * @return false
  ...
  /**
   * @brief Convert dynamic address of the executed executable to offset
   * address.
   *
   * @param call_path
   */
  ...
  /** Read dynamic program call graph. This function includes two phases:
   * indirect call relationship analysis, as well as pthread_create and its
   * created function.
   * @param perf_data - performance data
   */
  void ReadDynamicProgramCallGraph(core::PerfData *perf_data);

  /** Generate complete program call graph through hybrid static-dynamic
   * analysis.
   * @param pcg_name - program call graph file name
   * @param perf_data - performance data
   * @param shared_obj_map_file_name - file name of shared object map
   */
  void GenerateProgramCallGraph(const char *pcg_name,
                                core::PerfData *perf_data);
  ...
  /** Perform intra-procedural analysis to abstract program structure from
   * control-flow graph. Not be implemented yet.
   */
  void IntraProceduralAnalysis();

  /** Read function abstraction graphs of all functions in a program from a
   * directory.
   ...
   */
  core::ProgramAbstractionGraph *
  GetFunctionAbstractionGraph(type::addr_t entry_addr);

  /** Get function abstraction graph by address. (entry address of this function
   * <= input address <= exit address of this function)
   * @param addr - input address for identification
   * @return program abstraction graph of the identified function
   */
  core::ProgramAbstractionGraph *
  GetFunctionAbstractionGraphByAddr(type::addr_t addr);

  /** Get function abstraction graphs of all functions.
   * @return a map of function names and corresponding fucntion abstraction
   * graphs
   */
  std::map<type::addr_t, core::ProgramAbstractionGraph *> &
  ...
  /** Perform dynamic inter-procedural analysis. Add pag of created function to
   * pthread_create vertex.
  ...
  /** Perform inter-procedural analysis. Add pag of callee function to call
   * vertex.
  ...
  /** Generate an overall program abstraction graph through intra-procedural
   * analysis and inter-procedural analysis. param binary_name - name of binary
   * for static analysis
  ...
  /** Set a input pag as program abstraction graph. This function is for reusing
   * pag after intra-procedural and inter-procedural analysis.
   ...
   */
  type::vertex_t GetVertexWithInterThreadAnalysis(int thread_id,
  ...
  void OpenMPGroupThreadPerfData(std::string &metric,
  ...
  void
  GenerateMultiProcessProgramAbstractionGraph(type::vertex_t starting_vertex,
                                              int num_procs);
