...
namespace std {
template <typename T1, typename T2> struct less<std::pair<T1, T2>> {
...
std::map<type::thread_t, std::pair<type::call_path_t, type::thread_t>>
    created_tid_2_callpath_and_tid;
...
void build_create_tid_to_callpath_and_tid(core::PerfData *perf_data) {
  ...
  for (unsigned long int i = 0; i < edge_data_size; i++) {
    /** Value of pthread_create is recorded as (-1), Value of GOMP_parallel is
     * recorded as (-2) */
...
void GPerf::GenerateControlFlowGraphs(const char *dir_name) {
  this->ReadStaticControlFlowGraphs(dir_name);
}
...
std::map<type::addr_t, core::ControlFlowGraph *> &
GPerf::GetControlFlowGraphs() {
  return this->func_entry_addr_to_cfg;
}

void GPerf::GenerateDynAddrDebugInfo(
    core::PerfData *perf_data,
    std::map<type::procs_t, collector::SharedObjAnalysis *>
  ...
  for (auto &kv : all_addrs) {
    ...
    all_shared_obj_analysis[procs_id]->GetDebugInfos(
        addrs, this->dyn_addr_to_debug_info, binary_name);
  }
...
void GPerf::GenerateDynAddrDebugInfo(core::PerfData *perf_data,
                                     std::string &shared_obj_map_file_name,
                                     std::string &binary_name) {
  std::map<type::procs_t, collector::SharedObjAnalysis *>
      all_shared_obj_analysis;
  collector::SharedObjAnalysis *shared_obj_analysis =
...
void GPerf::GenerateDynAddrDebugInfo(core::PerfData *perf_data,
  ...
  GenerateDynAddrDebugInfo(perf_data, shared_obj_map_file_name,
                           binary_name_str);
} // GPerf::GenerateDynAddrDebugInfo

void GPerf::GenerateDynAddrDebugInfo(core::PerfData *perf_data,
                                     type::procs_t num_procs,
                                     std::string &binary_name) {
  std::map<type::procs_t, collector::SharedObjAnalysis *>
      all_shared_obj_analysis;
  for (type::procs_t i = 0; i < num_procs; i++) {
    std::string somap_file_name_str =
        std::string("SOMAP+") + std::to_string(i) + std::string(".TXT");
    collector::SharedObjAnalysis *shared_obj_analysis =
        new baguatool::collector::SharedObjAnalysis();
...
std::map<type::addr_t, type::addr_debug_info_t *> &
...
void SetCallTypeAsStatic(core::ProgramCallGraph *pcg, type::edge_t edge_id,
                         void *extra) {
...
void GPerf::ReadStaticProgramCallGraph(const char *pcg_name) {
  ...
  std::string static_pcg_file_name =
      std::string(pcg_name); //+ std::string(".pcg");
...
void GPerf::ReadDynamicProgramCallGraph(core::PerfData *perf_data) {
  ...
  //           if (dyn_addr_to_debug_info.find(callee_addr) !=
  //               dyn_addr_to_debug_info.end()) {
  //             // dbg(callee_addr);
  //             break;
  //           }
  //         }
  //         call_path.pop();
  //       }
  //       /** For cases that all addresses are popped. Test whether the last
  //        * address has debug info */
  //       if (type::IsDynAddr(callee_addr) &&
  //           dyn_addr_to_debug_info.find(callee_addr) ==
  //               dyn_addr_to_debug_info.end()) {
  //         continue;
  //       }

  //       std::pair<type::addr_t, type::addr_t> call_callee_pair =
  //           std::make_pair(call_addr, callee_addr);
  //       if (visited_call_callee.find(call_callee_pair) !=
  //           visited_call_callee.end()) {
  //         continue;
  //       }
  //       // dbg(call_addr, callee_addr);
  ...
  //       } else {
  //         type::vertex_t call_vertex =
  //             this->pcg->GetCallVertexWithAddr(call_addr);
  //         if (call_vertex != -1) {
  //           // dbg("find call vertex", call_vertex);

  //           std::string &func_name =
  //               dyn_addr_to_debug_info[callee_addr]->GetFuncName();
  //           if (func_name.empty()) {
  //             continue;
  //           }
  //           // dbg(func_name);
  ...
  //           type::edge_t new_func_edge_id =
  //               this->pcg->AddEdge(call_vertex, new_func_vertex_id);
  //           this->pcg->SetEdgeType(new_func_edge_id,
  ...
  //           type::edge_t new_call_edge_id =
  //               this->pcg->AddEdge(new_func_vertex_id, new_call_vertex_id);
  ...
  //           new_pag->SetVertexBasicInfo(new_func_vertex_id, type::FUNC_NODE,
  //                                       func_name.c_str());
  //           new_pag->SetVertexDebugInfo(new_func_vertex_id, callee_addr,
  //                                       callee_addr);

  //           new_call_vertex_id = new_pag->AddVertex();
  //           new_pag->SetVertexBasicInfo(new_call_vertex_id, type::CALL_NODE,
  //                                       "CALL_SO");
  //           new_pag->SetVertexDebugInfo(new_call_vertex_id, callee_addr,
  //                                       callee_addr);
...
void GPerf::ReadFunctionAbstractionGraphs(const char *dir_name) {
  ...
  for (const auto &hash_str : file_names) {
    ...
    /**
     * Extract entry_addr from ./[bin_name]/[hash_str].pag and
     * [bin_name].pag.map ([hash_str, entry_addr])
     */
...
core::ProgramAbstractionGraph *
GPerf::GetFunctionAbstractionGraph(type::addr_t func_entry_addr) {
  return this->func_entry_addr_to_pag[func_entry_addr];
}

std::map<type::addr_t, core::ProgramAbstractionGraph *> &
GPerf::GetFunctionAbstractionGraphs() {
  return this->func_entry_addr_to_pag;
}

core::ProgramAbstractionGraph *
GPerf::GetFunctionAbstractionGraphByAddr(type::addr_t addr) {
...
void ConnectCallerCallee(core::ProgramAbstractionGraph *pag, int vertex_id,
                         void *extra) {
  ...
  if (type == type::CALL_NODE || type == type::CALL_IND_NODE ||
    ...
    for (auto callee_func_entry_addr : callee_func_entry_addrs) {
      for (type::addr_t entry_addr = callee_func_entry_addr - 4;
           entry_addr <= callee_func_entry_addr + 4; entry_addr++) {
        if (func_entry_addr_to_pag->find(entry_addr) !=
            func_entry_addr_to_pag->end()) {
          core::ProgramAbstractionGraph *callee_pag =
              (*func_entry_addr_to_pag)[entry_addr];
          // printf("%s \n", callee_pag->GetGraphAttributeString("name"));

          if (!callee_pag->GetGraphAttributeFlag("scanned")) {
            void (*ConnectCallerCalleePointer)(core::ProgramAbstractionGraph *,
                                               int, void *) =
                &(ConnectCallerCallee);
          ...
          // pag->SetVertexAttributeString("name", vertex_id,
          // callee_pag->GetGraphAttributeString("name"));
        }
      }
    }
...
void GPerf::DynamicInterProceduralAnalysis(core::PerfData *pthread_data) {
  ...
  // Only focus on adding relationships between pthread_create and created
  // functions
  ...
  for (unsigned long int i = 0; i < edge_data_size; i++) {
    ...
    if (value == (type::perf_data_t)(-1)) {
      ...
      // type::vertex_t queried_vertex_id =
      // this->root_pag->GetVertexWithCallPath(0, src_call_path);
      ...
      if (!src_call_path.empty()) {
        ...
        root_pag->SetVertexBasicInfo(pthread_create_vertex_id, type::CALL_NODE,
                                     "pthread_create");
        root_pag->SetVertexDebugInfo(pthread_create_vertex_id, addr, addr);
        root_pag->SetVertexAttributeNum("id", pthread_create_vertex_id,
                                        pthread_create_vertex_id);
      ...
      for (unsigned long int j = i + 1; j < edge_data_size; j++) {
        // dbg(j, edge_data_size, dest_thread_id,
        // pthread_data->GetEdgeDataSrcThreadId(j));
        ...
        if (dest_thread_id == pthread_data->GetEdgeDataSrcThreadId(j)) {
          ...
          // this->root_pag->GetVertexWithCallPath(0, dest_call_path_join);
          type::vertex_t queried_vertex_id_join =
              GetVertexWithInterThreadAnalysis(dest_thread_id_join,
                                               dest_call_path_join);
          ...
          if (!src_call_path.empty()) {
            ...
            root_pag->SetVertexBasicInfo(pthread_join_vertex_id,
                                         type::CALL_NODE, "pthread_join");
            root_pag->SetVertexDebugInfo(pthread_join_vertex_id, addr_join,
                                         addr_join);
            // dbg(root_pag->GetCurVertexNum());
            root_pag->SetVertexAttributeNum("id", pthread_join_vertex_id,
                                            pthread_join_vertex_id);
          ...
          this->root_pag->GetGraphPerfData()->SetPerfData(
              pthread_join_vertex_id, metric,
              pthread_data->GetEdgeDataDestProcsId(j), dest_thread_id_join,
              join_value);
          // this->graph_perf_data->SetPerfData(pthread_join_vertex_id,
          // perf_data->GetMetricName(), process_id, thread_id, data);
      ...
      for (unsigned long int j = 0; j < vertex_data_size; j++) {
        // find the samples with same thread id as created thread id of this
        // pthread_create
        ...
        if (func_thread_id == dest_thread_id) {
          ...
          // Use GetVertexWithCallPath function to pop address that over
          // 0x7ff0000000, it must return 0 - root of pag
      ...
      if (func_vertex_id == -1) {
        ...
        this->root_pag->SetVertexAttributeNum("wait", pthread_join_vertex_id,
                                              func_vertex_id);
...
void GPerf::GenerateProgramAbstractionGraph(core::PerfData *perf_data) {
  this->InterProceduralAnalysis(perf_data);
}

void GPerf::GenerateStaticProgramAbstractionGraph() {
  this->StaticInterProceduralAnalysis();
}

void GPerf::SetProgramAbstractionGraph(core::ProgramAbstractionGraph *pag) {
  this->root_pag = pag;
}

core::ProgramAbstractionGraph *GPerf::GetProgramAbstractionGraph() {
  return this->root_pag;
}

type::vertex_t
GPerf::GetVertexWithInterThreadAnalysis(type::thread_t thread_id,
  ...
  std::pair<type::call_path_t, type::thread_t> next_pair =
      created_tid_2_callpath_and_tid[thread_id];
  ...
  auto starting_vertex =
      GetVertexWithInterThreadAnalysis(parent_thread_id, parent_call_path);
  ...
  auto detected_vertex =
      this->root_pag->GetVertexWithCallPath(starting_vertex, call_path);
...
void in_pthread_expansion(core::ProgramAbstractionGraph *pag, int vertex_id,
  ...
  if (strcmp(pag->GetVertexAttributeString("name", vertex_id),
             "pthread_join") == 0) {
    if (pag->HasVertexAttribute("wait")) {
      type::vertex_t wait_vertex_id =
          pag->GetVertexAttributeNum("wait", vertex_id);

      if (wait_vertex_id > 0) {
        type::vertex_t last_vertex_id =
            pag->GetVertexAttributeNum("last", wait_vertex_id);
...
void out_pthread_expansion(core::ProgramAbstractionGraph *pag, int vertex_id,
                           void *extra) {
  struct pthread_expansion_arg_t *arg = (struct pthread_expansion_arg_t *)extra;
  // core::MultiProgramAbstractionGraph *mpag = arg->mpag;
  std::map<type::vertex_t, type::vertex_t> *pag_vertex_id_2_mpag_vertex_id =
  ...
  if (strcmp(pag->GetVertexAttributeString("name", parent_vertex_id),
             "pthread_create") == 0) {
...
void add_unlock_to_lock_edge(core::MultiProgramAbstractionGraph *pag,
    ...
    if (pag->HasVertexAttribute("wait")) {
      ...
      if (wait_vertex_id > 0) {
        type::vertex_t dest_vertex_id =
            (*pag_vertex_id_2_mpag_vertex_id)[wait_vertex_id];

        // dbg(vertex_id, pag->GetVertexAttributeString("name",
        // dest_vertex_id));
...
void GPerf::GenerateMultiThreadProgramAbstractionGraph() {
  ...
  this->root_mpag->VertexTraversal(add_unlock_to_lock_edge,
                                   pag_vertex_id_2_mpag_vertex_id);
...
void group_thread_perf_data(core::ProgramAbstractionGraph *pag, int vertex_id,
                            void *extra) {
  struct group_thread_perf_data_arg_t *arg =
      (struct group_thread_perf_data_arg_t *)extra;
  ...
  pag_graph_perf_data->GetMetricsPerfDataProcsNum(vertex_id, arg->metric,
                                                  procs_vec);
  for (auto procs : procs_vec) {
    /** Get original process performance data */
    std::map<type::thread_t, type::perf_data_t> proc_perf_data;
    pag_graph_perf_data->GetProcsPerfData(vertex_id, arg->metric, procs,
                                          proc_perf_data);
    ...
    pag_graph_perf_data->SetProcsPerfData(vertex_id, arg->metric, procs,
                                          group_proc_perf_data);
...
void GPerf::OpenMPGroupThreadPerfData(std::string &metric,
                                      type::thread_t num_groups) {
  auto pag_graph_perf_data = this->root_pag->GetGraphPerfData();
  struct group_thread_perf_data_arg_t *arg =
      new (struct group_thread_perf_data_arg_t)();
...
void in_openmp_expansion(core::ProgramAbstractionGraph *pag, int vertex_id,
                         void *extra) {
...
void out_openmp_expansion(core::ProgramAbstractionGraph *pag, int vertex_id,
                          void *extra) {
  struct openmp_expansion_arg_t *arg = (struct openmp_expansion_arg_t *)extra;
  core::MultiProgramAbstractionGraph *mpag = arg->mpag;
  ...
  if (openmp_seq_record_flag) {
    ...
    if (strcmp(pag->GetVertexAttributeString("name", parent_vertex_id),
      ...
      mpag->SetVertexAttributeString("name", end_vertex_id,
                                     "GOMP_parallel.end");
      ...
      for (auto vertex_id : openmp_seq) {
        ...
        for (auto metric : metrics) {
          std::vector<type::procs_t> procs_vec;
          pag_graph_perf_data->GetMetricsPerfDataProcsNum(vertex_id, metric,
                                                          procs_vec);
          for (auto procs_id : procs_vec) {
            std::map<type::thread_t, type::perf_data_t> proc_perf_data;
            pag_graph_perf_data->GetProcsPerfData(vertex_id, metric, procs_id,
                                                  proc_perf_data);
            ...
            for (auto &thread_perf_data : proc_perf_data) {
              ...
              mpag_graph_perf_data->SetPerfData(
                  new_vertex_set[i][j], metric, procs_id,
                  thread_perf_data.first, thread_perf_data.second);
              i++;
            }
            FREE_CONTAINER(proc_perf_data);
          }
...
void GPerf::AddCommEdgesToMPAG(core::PerfData *comm_data) {
  ...
  for (unsigned long int i = 0; i < edge_data_size; i++) {
    ...
    if (value > COMM_TIME_THRD) {
      ...
      type::vertex_t queried_vertex_id_dest =
          GetVertexWithInterThreadAnalysis(0, dest_call_path);
...
void in_pre_order_traversal(core::ProgramAbstractionGraph *pag, int vertex_id,
                            void *extra) {
...
void GPerf::GenerateMultiProcessProgramAbstractionGraph(
    type::vertex_t starting_vertex, int num_procs) {
  ...
  for (int i = 0; i < num_procs; i++) {
    ...
    for (auto vertex_id : *pre_order_vertex_seq) {
      ...
      for (auto metric : metrics) {
        std::map<type::thread_t, type::perf_data_t> proc_perf_data;
        pag_graph_perf_data->GetProcsPerfData(vertex_id, metric, i,
                                              proc_perf_data);
        mpag_graph_perf_data->SetProcsPerfData(new_vertex_id, metric, i,
                                               proc_perf_data);
        FREE_CONTAINER(proc_perf_data);
      }
...
core::MultiProgramAbstractionGraph *GPerf::GetMultiProgramAbstractionGraph() {
  return root_mpag;
}
