#!/bin/bash
# PerFlow Python Module Setup Diagnostic Script
# Run this from the PerFlow root directory to diagnose import issues
...
if [ -f "build/python/perflow/_perflow_bindings.so" ]; then
    ...
    ls -lh build/python/perflow/_perflow_bindings.cpython-*.so 2>/dev/null | head -1
else
...
if [ -d "build/python/perflow" ]; then
    ...
    ls -la build/python/perflow/ 2>/dev/null | grep -v "^total"
else
...
if [ -d "python/perflow" ]; then
    ...
    ls -1 python/perflow/*.py 2>/dev/null
else
...
try:
    ...
    build_perflow = os.path.join(os.getcwd(), 'build', 'python', 'perflow')
    if os.path.exists(build_perflow):
        print(f"   Files in {build_perflow}:")
        for f in os.listdir(build_perflow):
            print(f"     - {f}")
EOF
echo
echo "======================================"
...
echo "======================================"
echo
echo "If import failed, try:"
echo "  1. Make sure you're in the PerFlow root directory"
echo "  2. Run: export PYTHONPATH=\$PWD/build/python:\$PYTHONPATH"
echo "  3. Run: python3 -c 'import perflow; print(\"Success!\")'"
