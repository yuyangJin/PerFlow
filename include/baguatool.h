...
typedef std::stack<type::addr_t>
    call_path_t; /**<call path type (need to modify)*/
...
struct addr_debug_info_t {
  ...
  void SetFuncName(std::string &func_name) {
    this->func_name = std::string(func_name);
  }
 ...
 * @return true stands for the input address is in .text segment while false
 * means it is not.
 */
 ...
 * @param addr - input address.
 * @return true stands for the input address is in .dynamic segment while false
 * means it is not.
 */
 ...
 * @param addr - input address.
 * @return true stands for the input address is a valid address while false
 * means it is not.
 */
...
public:
  ...
  /** Record a piece of data of a specific vertex, including metric, process id,
   * thread id, and value.
   ...
   * @param value - value
   */
  void SetPerfData(type::vertex_t vertex_id, std::string &metric, int procs_id,
                   int thread_id, baguatool::type::perf_data_t value);

  /** Query the value of a piece of data of a specific vertex through metric,
   ...
   * @return value
   */
  baguatool::type::perf_data_t GetPerfData(type::vertex_t vertex_id,
                                           std::string &metric, int procs_id,
                                           int thread_id);
   ...
   */
  void GetVertexPerfDataMetrics(type::vertex_t vertex_id,
                                std::vector<std::string> &);
  int GetMetricsPerfDataProcsNum(type::vertex_t vertex_id, std::string &metric,
                                 std::vector<type::procs_t> &procs_list);
  int GetProcsPerfDataThreadNum(type::vertex_t vertex_id, std::string &metric,
                                int procs_id,
                                std::vector<type::thread_t> &thread_vec);
  void
  GetProcsPerfData(type::vertex_t vertex_id, std::string &metric, int procs_id,
                   std::map<type::thread_t, type::perf_data_t> &proc_perf_data);
  void
  SetProcsPerfData(type::vertex_t vertex_id, std::string &metric, int procs_id,
                   std::map<type::thread_t, type::perf_data_t> &proc_perf_data);

  void EraseProcsPerfData(type::vertex_t vertex_id, std::string &metric,
                          int procs_id);
}; // class GraphPerfData
...
public:
   ...
   * @return 0 is success
   */
  int SetVertexBasicInfo(const type::vertex_t vertex_id, const int vertex_type,
                         const char *vertex_name);

  /** Set debug information of a vertex, including address or line number (need
   * to extend).
   * @param vertex_id - id of the target vertex
   ...
   * @return 0 is success
   */
  int SetVertexDebugInfo(const type::vertex_t vertex_id,
  ...
  /** Identify the vertex corresponding to the address from a specific starting
   * vertex.
   * @param root_vertex_id - id of the starting vertex
   * @param addr - address
   * @return id of the identified vertex
   */
  type::vertex_t GetChildVertexWithAddr(type::vertex_t root_vertex_id,
                                        type::addr_t addr);

  /** Identify the vertex corresponding to the call path from a specific
   * starting vertex.
   * @param root_vertex_id - id of the starting vertex
   ...
   */
  type::vertex_t
  GetVertexWithCallPath(type::vertex_t root_vertex_id,
  ...
  /** Identify the function vertex corresponding to the address from all
   * vertices.
   * @param addr - address
   * @return id of the identified vertex
   */
  ...
  /** Add a new edge between call vertex and callee function vertex through
   * their addresses.
  ...
  /** Lazy add a new edge between call vertex and callee function vertex through
   * their addresses.
   * @param call_addr - address of the call instruction
   * @param callee_addr - address of the callee function
   * @return
  ...
  /** Get name of a vertex's callee vertex, call edge type must be the input
   * type
   * @param vertex_id - id of vertex
  ...
  /** Get entry address of a vertex's callee vertex, and edge type must be the
   * input type
   * @param vertex_id - id of vertex
   * @param input_type - the input type
   * @return entry address of the callee vertex
   ...
   */
  void GetCalleeEntryAddrs(type::vertex_t vertex_id,
   ...
   * parameter of CALL_BACK_FUNC
   */
  void VertexTraversal(void (*CALL_BACK_FUNC)(ProgramGraph *, int, void *),
                       void *extra);

  /** [Graph Algorithm] Traverse all edges and execute CALL_BACK_FUNC when
   * accessing each edge.
   * @param CALL_BACK_FUNC - callback function when an edge is accessed. The
   * input parameters of this function contain a pointer to the graph being
   * traversed, id of the accessed edge, and an extra pointer for developers to
   * pass more parameters.
   * @param extra - a pointer for developers to pass more parameters as the last
   * parameter of CALL_BACK_FUNC
   */
  void EdgeTraversal(void (*CALL_BACK_FUNC)(ProgramGraph *, int, void *),
                     void *extra);
...
public:
   ...
   * parameter of CALL_BACK_FUNC
   */
  void VertexTraversal(void (*CALL_BACK_FUNC)(ProgramAbstractionGraph *, int,
                                              void *),
                       void *extra);
   ...
   * parameter of CALL_BACK_FUNC
   */
  void EdgeTraversal(void (*CALL_BACK_FUNC)(ProgramAbstractionGraph *, int,
                                            void *),
                     void *extra);
   ...
   *
   */
  void DFS(type::vertex_t root,
           void (*IN_CALL_BACK_FUNC)(ProgramAbstractionGraph *, int, void *),
           void (*OUT_CALL_BACK_FUNC)(ProgramAbstractionGraph *, int, void *),
           void *extra);

  /** Reduce performance data of each process and thread of a speific metric for
   * each vertex (GraphPerfData)
   * @param metric - a specfic metric
   * @param op - reduce operation
   * @return what????
   */
  baguatool::type::perf_data_t ReduceVertexPerfData(std::string &metric,
                                                    std::string &op);

  /** Convert performance data to percentage format for each vertex (total value
   * as 100 percent).
   * @param metric - a specfic metric
   * @param total - total value
   * @param new_metric - new metric for record percentage format data
   */
  void ConvertVertexReducedDataToPercent(std::string &metric,
...
public:
   ...
   * parameter of CALL_BACK_FUNC
   */
  void VertexTraversal(void (*CALL_BACK_FUNC)(MultiProgramAbstractionGraph *,
                                              int, void *),
                       void *extra);
   ...
   * parameter of CALL_BACK_FUNC
   */
  void EdgeTraversal(void (*CALL_BACK_FUNC)(MultiProgramAbstractionGraph *, int,
                                            void *),
                     void *extra);
...
class PerfData {
private:
  VDS *vertex_perf_data =
      nullptr; /**<pointer to vertex type performance data */
  unsigned long int vertex_perf_data_space_size =
      0; /**<pre-allocate space size of vertex type performance data*/
  unsigned long int vertex_perf_data_count =
      0; /**<amount of recorded vertex type performance data*/
  EDS *edge_perf_data = nullptr; /**<pointer to edge type performance data */
  unsigned long int edge_perf_data_space_size =
      0; /**<pre-allocate space size of edge type performance data*/
  unsigned long int edge_perf_data_count =
      0; /**<amount of recorded edge type performance data*/
  FILE *perf_data_fp = nullptr;    /**<file handler for output */
  std::ifstream perf_data_in_file; /**<file handler for input */
  bool has_open_output_file =
      false; /**<flag for record whether output file has open or not*/
...
public:
  ...
  /** Query a piece of vertex type performance data by (call path, call path
   * length, process id, thread id)
   * @param call_path - call path
   ...
   */
  int QueryVertexData(baguatool::type::addr_t *call_path, int call_path_len,
                      int procs_id, int thread_id);

  /** Query a piece of edge type performance data by (call path, call path
   * length, process id, thread id)
   ...
   * @return index of the queried piece of data
   */
  int QueryEdgeData(baguatool::type::addr_t *call_path, int call_path_len,
                    baguatool::type::addr_t *out_call_path,
                    int out_call_path_len, int procs_id, int out_procs_id,
                    int thread_id, int out_thread_id);
   ...
   */
  void RecordVertexData(baguatool::type::addr_t *call_path, int call_path_len,
                        int procs_id, int thread_id,
   ...
   * @param value of this piece of data
   */
  void RecordEdgeData(baguatool::type::addr_t *call_path, int call_path_len,
                      baguatool::type::addr_t *out_call_path,
                      int out_call_path_len, int procs_id, int out_procs_id,
                      int thread_id, int out_thread_id,
                      baguatool::type::perf_data_t value);
  ...
  //  */
  // void SetVertexDataCallPath(unsigned long int data_index,
  // std::stack<type::addr_t>& call_path_stack);

  /** Query source call path of a piece of edge type performance data through
   * index
   * @param data_index - index of the piece of data
   * @param call_path - call path of the queried piece of data
   */
  void GetEdgeDataSrcCallPath(unsigned long int data_index,
                              std::stack<unsigned long long> &call_path);

  /** Query destination call path of a piece of edge type performance data
   * through index
   * @param data_index - index of the piece of data
   * @param call_path - call path of the queried piece of data
   */
  void GetEdgeDataDestCallPath(unsigned long int data_index,
                               std::stack<unsigned long long> &call_path);
  ...
  /** Query source process id of a piece of edge type performance data through
   * index
   * @param data_index - index of the piece of data
   * @return process id of the queried piece of data
   */
  ...
  /** Query destination process id of a piece of edge type performance data
   * through index
   * @param data_index - index of the piece of data
   * @return process id of the queried piece of data
   */
  ...
  /** Query source thread id of a piece of edge type performance data through
   * index
   * @param data_index - index of the piece of data
   * @return thread id of the queried piece of data
   */
  ...
  /** Query destination thread id of a piece of edge type performance data
   * through index
   * @param data_index - index of the piece of data
   * @return thread id of the queried piece of data
   */
...
//  private:
//   std::map<std::string, ControlFlowGraph*> func_cfg_map; /**<control-flow
//   graphs for each function*/ ProgramCallGraph* pcg; /**<program call graph*/
//   std::map<std::string, ProgramAbstractionGraph*>
//       func_pag_map; /**<program abstraction graph extracted from control-flow
//       graph (CFG) for each function */
//   ProgramAbstractionGraph* root_pag;  /**<an overall program abstraction
//   graph for a program */ ProgramAbstractionGraph* root_mpag; /**<an overall
//   multi-* program abstraction graph for a parallel program*/ GraphPerfData*
//   graph_perf_data;     /**<performance data in a graph*/
...
//   ProgramAbstractionGraph* GetFunctionAbstractionGraph(std::string
//   func_name); std::map<std::string, ProgramAbstractionGraph*>&
//   GetFunctionAbstractionGraphs(); void IntraProceduralAnalysis(); void
//   ReadFunctionAbstractionGraphs(const char* dir_name);
...
//   GraphPerfData* GetGraphPerfData();
//   baguatool::type::perf_data_t ReduceVertexPerfData(std::string& metric,
//   std::string& op); void ConvertVertexReducedDataToPercent(std::string&
//   metric, baguatool::type::perf_data_t total, std::string& new_metric);
...
/** Static analysis based on binary. Extract structure and function call
 * relationship and ...
 *
 */
...
public:
  ...
  /** Set overflow function. Sampler executes an input function every time the
  ...
  /** Get overflow event.
   * @param overflow_vector - papi overflow vector to detect which event counter
   * is overflow
