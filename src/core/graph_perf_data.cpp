...
void GraphPerfData::SetPerfData(type::vertex_t vertex_id, std::string &metric,
  ...
  this->j_perf_data[vertex_id_str][metric_str][process_id_str][thread_id_str] =
      data;
}

type::perf_data_t GraphPerfData::GetPerfData(type::vertex_t vertex_id,
  ...
  if (this->j_perf_data.contains(vertex_id_str)) {
    if (this->j_perf_data[vertex_id_str].contains(metric_str)) {
      if (this->j_perf_data[vertex_id_str][metric_str].contains(
              process_id_str)) {
        if (this->j_perf_data[vertex_id_str][metric_str][process_id_str]
                .contains(thread_id_str)) {
          auto ret = this->j_perf_data[vertex_id_str][metric_str]
                                      [process_id_str][thread_id_str];
          if (ret != nullptr) {
            data += this->j_perf_data[vertex_id_str][metric_str][process_id_str]
                                     [thread_id_str]
                                         .get<type::perf_data_t>();
          }
        }
      }
    }
...
void GraphPerfData::GetVertexPerfDataMetrics(
    type::vertex_t vertex_id, std::vector<std::string> &metrics) {
  std::string vertex_id_str = std::to_string(vertex_id);
...
int GraphPerfData::GetMetricsPerfDataProcsNum(
    type::vertex_t vertex_id, std::string &metric,
  ...
  for (auto &procs_data :
       this->j_perf_data[vertex_id_str][metric_str].items()) {
...
int GraphPerfData::GetProcsPerfDataThreadNum(
    type::vertex_t vertex_id, std::string &metric, int procs_id,
  ...
  int size =
      this->j_perf_data[vertex_id_str][metric_str][process_id_str].size();
  for (auto &thread_data :
       this->j_perf_data[vertex_id_str][metric_str].items()) {
...
// type::perf_data_t** GraphPerfData::GetMetricPerfData(type::vertex_t
// vertex_id, std::string& metric) {}
void GraphPerfData::GetProcsPerfData(
    type::vertex_t vertex_id, std::string &metric, int procs_id,
  ...
  for (auto &thread_perf_data :
       j_perf_data[vertex_id_str][metric_str][process_id_str].items()) {
    ...
    // if
    // (this->j_perf_data[vertex_id_str][metric_str][process_id_str].contains(thread_id_str))
    // {
  ...
  // int size =
  // this->j_perf_data[vertex_id_str][metric_str][process_id_str].size(); auto
  // this->j_perf_data[vertex_id_str][metric_str][process_id_str].keys()
...
void GraphPerfData::SetProcsPerfData(
    type::vertex_t vertex_id, std::string &metric, int procs_id,
    std::map<type::thread_t, type::perf_data_t> &proc_perf_data) {
  std::string vertex_id_str = std::to_string(vertex_id);
  std::string metric_str = std::string(metric);
  ...
  for (auto &kv : proc_perf_data) {
    ...
    this->j_perf_data[vertex_id_str][metric_str][process_id_str]
                     [thread_id_str] = kv.second;
  }
  return;
}

void GraphPerfData::EraseProcsPerfData(type::vertex_t vertex_id,
                                       std::string &metric, int procs_id) {
  std::string vertex_id_str = std::to_string(vertex_id);
  std::string metric_str = std::string(metric);
  std::string process_id_str = std::to_string(procs_id);
