...
void HybridAnalysis::ReadStaticControlFlowGraphs(const char *dir_name) {
  ...
  for (const auto &fn : file_names) {
    ...
    this->func_cfg_map[std::string(func_cfg->GetGraphAttributeString("name"))] =
        func_cfg;
  }
}

void HybridAnalysis::GenerateControlFlowGraphs(const char *dir_name) {
  this->ReadStaticControlFlowGraphs(dir_name);
}

ControlFlowGraph *HybridAnalysis::GetControlFlowGraph(std::string func_name) {
  return this->func_cfg_map[func_name];
}

std::map<std::string, ControlFlowGraph *> &
HybridAnalysis::GetControlFlowGraphs() {
  return this->func_cfg_map;
}
...
void HybridAnalysis::ReadStaticProgramCallGraph(const char *binary_name) {
  ...
  std::string static_pcg_file_name =
      std::string(binary_name) + std::string(".pcg");
...
void HybridAnalysis::ReadDynamicProgramCallGraph(std::string perf_data_file) {
  ...
  /** Optimization: First scan all call path and store <call_addr, callee_addr>
   * pairs, then AddEdgeWithAddr. It can reduce redundant graph query **/
...
void HybridAnalysis::ReadFunctionAbstractionGraphs(const char *dir_name) {
  ...
  for (const auto &fn : file_names) {
    ...
    // new_pag->DumpGraph((file_name + std::string(".bak")).c_str());
    func_pag_map[std::string(new_pag->GetGraphAttributeString("name"))] =
        new_pag;
  }
}
...
HybridAnalysis::GetFunctionAbstractionGraph(std::string func_name) {
  return this->func_pag_map[func_name];
}

std::map<std::string, ProgramAbstractionGraph *> &
HybridAnalysis::GetFunctionAbstractionGraphs() {
...
void ConnectCallerCallee(ProgramAbstractionGraph *pag, int vertex_id,
  ...
  if (type == CALL_NODE || type == CALL_IND_NODE || type == CALL_REC_NODE) {
    ...
    //     (*func_name_2_pag)[std::string(pag->GetVertexAttributeString("name",
    //     vertex_id))];
    ...
    if (callee_func_name) {
      ProgramAbstractionGraph *callee_pag =
          (*func_name_2_pag)[callee_func_name_str];

      if (!callee_pag->GetGraphAttributeFlag("scanned")) {
        void (*ConnectCallerCalleePointer)(ProgramAbstractionGraph *, int,
                                           void *) = &(ConnectCallerCallee);
...
void HybridAnalysis::InterProceduralAnalysis() {
  ...
  // void (*ConnectCallerCalleePointer)(Graph *, int, void *) =
  // &ConnectCallerCallee; dbg(this->root_pag->GetGraphAttributeString("name"));
...
void HybridAnalysis::SetProgramAbstractionGraph(ProgramAbstractionGraph *pag) {
  this->root_pag = pag;
}

ProgramAbstractionGraph *HybridAnalysis::GetProgramAbstractionGraph() {
...
void HybridAnalysis::DataEmbedding(PerfData *perf_data) {
  ...
  for (unsigned long int i = 0; i < data_size; i++) {
    ...
    perf_data_t data = this->graph_perf_data->GetPerfData(
        queried_vertex_id, perf_data->GetMetricName(), process_id, thread_id);
    data += value;
    this->graph_perf_data->SetPerfData(queried_vertex_id,
                                       perf_data->GetMetricName(), process_id,
                                       thread_id, data);
  }

} // function Dataembedding

GraphPerfData *HybridAnalysis::GetGraphPerfData() {
  return this->graph_perf_data;
}

perf_data_t ReduceOperation(std::vector<perf_data_t> &perf_data, int num,
                            string &op) {
  if (num == 0) {
    return 0.0;
  }
...
void ReducePerfData(ProgramAbstractionGraph *pag, int vertex_id, void *extra) {
  ...
  int num_procs =
      graph_perf_data->GetMetricsPerfDataProcsNum(vertex_id, metric);
...
perf_data_t HybridAnalysis::ReduceVertexPerfData(std::string &metric,
                                                 std::string &op) {
...
void HybridAnalysis::ConvertVertexReducedDataToPercent(
    std::string &metric, perf_data_t total, std::string &new_metric) {
...
ProgramAbstractionGraph *HybridAnalysis::GetMultiProgramAbstractionGraph() {
  return root_mpag;
}
...
void HybridAnalysis::PthreadAnalysis(PerfData *pthread_data) {
  ...
  for (unsigned long int i = 0; i < data_size; i++) {
    ...
    if (time == (baguatool::core::perf_data_t)(-1)) {
      ...
      root_pag->SetVertexBasicInfo(pthread_vertex_id, core::CALL_NODE,
                                   "pthread_create");
      root_pag->SetVertexDebugInfo(pthread_vertex_id, addr, addr);
      root_pag->SetVertexAttributeNum("id", pthread_vertex_id,
                                      pthread_vertex_id);
      ...
      for (unsigned long int j = 0; j < data_size; j++) {
        if (j != i) {
          if (create_thread_id == pthread_data->GetVertexDataProcsId(j)) {
            ...
            type::vertex_t queried_vertex_id_j =
                this->root_pag->GetVertexWithCallPath(0, call_path_j);
            ...
            root_pag->SetVertexBasicInfo(pthread_join_vertex_id,
                                         core::CALL_NODE, "pthread_join");
            root_pag->SetVertexDebugInfo(pthread_join_vertex_id, addr_j,
                                         addr_j);
